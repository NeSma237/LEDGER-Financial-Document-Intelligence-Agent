...
# ---------------------------------------------------------------------------
# Config (matches orchestrator's documented env vars / default port)
# ---------------------------------------------------------------------------
...
def is_validated(data: dict) -> bool:
    """The schema has no explicit 'validated' flag; treat any answer other than
    insufficient_evidence as a validated answer."""
    return data.get("answer_type") != "insufficient_evidence"


def build_evidence_body(data: dict) -> str:
    answer_type = data.get("answer_type", "unknown")
    validated = is_validated(data)
    validated_badge = "✅ validated" if validated else "⚠️ validation failed"
